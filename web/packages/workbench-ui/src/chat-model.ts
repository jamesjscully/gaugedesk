 ...
 *
 * The model is the orthogonal `global → archetype → per-chat` axis (pi-rpc.md §A6). These
 ...
 * - `thinking` — the reasoning-effort level (`off|minimal|low|medium|high|xhigh`, Pi's
 *   `--thinking`). Absent → Pi's per-model default; present → pinned for this chat.
 *
...
/** The chat's pinned reasoning-effort level (`""` = Pi's per-model default). */
 ...
 *  clears the key so Pi's per-model default applies; any level (including `"off"`, an
